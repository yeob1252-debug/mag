...
check("prefers-reduced-motion:reduce" in rebuild_css and ".sr-journey-stage{display:none}" in rebuild_css, "static reduced-motion route")
check("직접 알리는 사람으로" in home and "sr-hero-emphasis" not in home and "sr-v3-hero-accent" in rebuild_css, "hero uses text-only emphasis without rejected block")
check(all(label in home for label in ("Instagram", "TikTok", "YouTube", "NAVER Clip", "당근")), "five platform text marks disclosed")
check("sr-reactions" not in home, "no fabricated engagement treatment")
check(all(f'data-journey-copy="course-{state}"' in home for state in ("theory", "ai", "shoot", "edit", "publish")), "five scroll-led one-day course states")
check("8시간+" in home and "8시간 이상" in course and "협찬 매장 식사 실습" in course, "confirmed 8H+ and field practice facts")
...
check(all(f'data-journey-screen="owner-{state}"' in home for state in ("store", "creator", "connect", "dm")), "visual-first owner match states")
...
journey_html = home[home.index('<section class="sr-journey"'):home.index('<section class="sr-prevalue')]
check(journey_html.count('class="sr-phone-shell"') == 1 and journey_html.count('data-persistent-phone') == 1, "one persistent phone DOM through all chapters")
...
check('<div class="sr-prevalue-card">' in home and 'href="channel-start-check.html">무료로 내 맛집채널 유형 확인하기</a></div>' in home, "free diagnosis hook facts and CTA stay in one block")
